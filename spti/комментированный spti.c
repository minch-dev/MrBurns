/*++
Копилефт (с) 1992 МелкийСофт
Имя:
	spti.c
Назначение:
	Win32 приложение для прямого обращения к SCSI устройствам через IOCTLы
Среда:
...
//массив с типами шин и их количество
...
VOID
__cdecl
////колво аргументов и сами аргументы
main(int argc,__nullterminated char *argv[])
{
	...
	//если неправильное колво аргументов - выдать вспомогательный текст
	if ((argc < 2) || (argc > 3)) {
	...
	shareMode = FILE_SHARE_READ | FILE_SHARE_WRITE;  // по умолчанию
	accessMode = GENERIC_WRITE | GENERIC_READ;       // по умолчанию

	//если аргументов - 3, то определить, какой режим задан
	if (argc == 3) {
	...
	if (!status ) {
		...
		return;
																				...
																				//КОМАНДА №1
	//а если удаётся - выдать эти параметры
...
//sptwb - scsi path trough with buffer
//
...
//  а вот-таки и самая главная функция
